...
from pyrep.policies.maddpg_drone_att_priority import MADDPG
            ...
            if rank == 0:
                for m in range(args.env_size):
                    if d_sg_list[m]==1:
                        ...
                        buffer.add(state_list[m][:args.n_agents],actions[m], r_list[m][:args.n_agents], state_next_list[m][:args.n_agents])

                if buffer.num_experiences >= buffer.learn_start:
                    for agent in agents:
                        agent.prep_training(device='cpu')
                    transitions,w_id,e_id = buffer.getBatch(args.batch_size)
                    delta_sum = []
                    for agent in agents:
                        other_agents = agents.copy()
                        other_agents.remove(agent)
                        delta,tst = agent.train(transitions,other_agents,logger,args.use_gpu,w_id)
                        delta_sum.append(delta)
                        agent.prep_rollouts(device='cpu')
                    
                    delta_sum1 = torch.cat(delta_sum, dim=1).mean(dim=1)

                    buffer.update_priority(e_id, delta_sum1)
                    if tst % 10000 == 0: 
                        buffer.rebalance()
