...
import os
from pyrep.envs.bacterium_environment_mpi1 import Drone_Env
from pyrep.common.arguments import get_args
from mpi4py import MPI
from pyrep.policies.maddpg_drone_att1 import MADDPG
...
def generate_actions(args,rank,state_list,policy_c,noise,epsilon):
    ...
    if rank == 0:
        ...
        with torch.no_grad():
            ...
            for i,policy in enumerate(policy_c): 
                actions.append(policy.select_action(sc[i], noise, epsilon,args.use_gpu))               
        #n*[n_env,adim]
        # print(actions)
...
def run(args, env, agents,rank,comm):
    ...
    for i in range(max_episodes):
        ...
        for t in range(episode_limit):
            ...
            #start = time.time()
            actions = generate_actions(args, rank,state_list,agents,noise,epsilon)
