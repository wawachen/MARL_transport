...
import os
from pyrep.envs.bacterium_environment_mpi_RL_orca import Drone_Env
...
def run(args, env, agents):
    ...
    buffer = Buffer(args)
    log_path = args.save_dir+"/log_drone_MPI_V0_orca" 
    ...
    while 1:
        ...
        if (i % evaluate_frequency == 0 and i!=0) and not repeat_eval:
            ...
            reward_index+=1
            print("orca_evaluation_episode:{}".format(reward_index)) 
        else:
            for t in range(episode_limit):
                ...
                if i>=args.stage_t:
                    ...
                    s_next, r, done, planes, prefer_vels = env.step_orca(actions)

                # add transitons in buff and update policy
                if i >= args.stage_t:
                    buffer.store_episode1(s, actions, r, s_next,done)
                else:
                    buffer.store_episode(s, actions, r, s_next, planes, prefer_vels,done)
                ...
                if buffer.current_size >= args.batch_size:
                    ...
                    for agent in agents:
                        ...
                        if i>=args.stage_t:
                            ...
                            agent.train_ORCA(transitions,other_agents,logger,args.use_gpu,i)
...
def evaluation(args, env, agents):
    log_path = args.save_dir+"/log_drone_MPI_V0_bc_evaluation_table"
    logger = SummaryWriter(logdir=log_path) # used for tensorboard
    returns = []
    time_f = []
    s_r = []
...
if __name__ == '__main__':
    ...
    if not args.evaluate:
        args.save_dir = "./" + args.scenario_name + "/model_drone{}_orca".format(args.n_agents)+'/'+'field_size{}'.format(args.field_size)+'/'+ 'env'+strftime("%Y-%m-%d--%H:%M:%S", localtime())
    else:
        args.save_dir = "./" + args.scenario_name + "/model_drone{}_orca".format(args.n_agents)+'/'+'field_size{}'.format(args.field_size)+'/'+ 'env'
    args.use_gpu = False
