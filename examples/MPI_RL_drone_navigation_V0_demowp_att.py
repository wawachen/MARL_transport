...
from pyrep.policies.maddpg_drone_att_demowp_att import MADDPG
        ...
        if buffer.ready():
            ...
            for _ in range(args.pretrain_save_step):
                ...
                for agent in agents:
                    ...
                    prio = agent.train(batch_data, other_agents, logger, args.use_gpu, weights,step)
            ...
            if rank == 0:
                for m in range(args.env_size):
                    if d_sg_list[m]==1:
                        ...
                        buffer.add((state_list[m][:args.n_agents], actions[m], r_list[m][:args.n_agents], state_next_list[m][:args.n_agents], DATA_RUNTIME))
                ...
                if buffer.ready():
                    ...
                    for agent in agents:
                        other_agents = agents.copy()
                        other_agents.remove(agent)
                        prio = agent.train(batch_data, other_agents, logger, args.use_gpu, weights,i)
                        p_list += np.array(prio)
                        agent.prep_rollouts(device='cpu')
...
def generate_evaluation_curve(args,env,agents):
    log_path = args.save_dir+"/log_drone_MPI_V0_demowp_evaluation" 
    logger = SummaryWriter(logdir=log_path) # used for tensorboard
